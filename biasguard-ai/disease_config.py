...
def get_domain_diseases(domain: str) -> list:
    """All disease keys belonging to a given domain."""
    return [d for d, cfg in DISEASE_CONFIG.items() if cfg.get("domain") == domain]

def get_all_domains() -> list:
    """Every distinct domain currently registered (endocrinology today,
    cardiology/pulmonology/nephrology once teammates add their configs)."""
    return sorted(set(cfg.get("domain") for cfg in DISEASE_CONFIG.values() if cfg.get("domain")))

def get_domain_lab_panel(domain: str) -> list:
    """Union of every feature_col needed across all diseases in a domain.
    Used to tell the lab technician form which fields to show."""
    fields = set()
    for cfg in DISEASE_CONFIG.values():
        if cfg.get("domain") == domain:
            fields.update(cfg["feature_cols"])
